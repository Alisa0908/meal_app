<x-app-layout>
    <div class="container lg:w-1/2 md:w-4/5 w-11/12 mx-auto mt-8 px-8 bg-white shadow-md">
        ...
        <form action="{{ route('posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="rounded pt-3 pb-8 mb-4">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm mb-2" for="category_id">
                    カテゴリー
                </label>
                @foreach ($categories as $category)
                    <input type="radio" id="category_id" name="category_id" value="{{ $category->id }}" @if (old('category_id', $post->category_id) == $category->id) checked @endif>
